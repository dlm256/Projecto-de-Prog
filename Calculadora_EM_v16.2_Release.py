...
#Segurança alternativa, não usada correntemente
...
#Segurança em desempenho correntemente
...
def guardar_pass(password):
    ...
    os.chmod(PASSWORD_FILE, stat.S_IRUSR | stat.S_IWUSR) #Sets permissions on PASSWORD_FILE - os.chmod(path,mode for permissions)
...
def definir_pw():
    while True:
        nova_password = reg_pass("Definir password: ")
...
#Fim de Segurança em utilização corrente em conjuncção com def reg_pass(prompt="Password: ")
...
def auth_user():
    ...
    logging.warning("Auth Failed! Too many attempts!")
    exit()
